...
class GptService(
    ...
    fun generateQuiz(requestContentDto: RequestContentDto): List<GptQuizDto> {
        val promptMessage =
            mutableListOf(
                Message(role = "system", content = "너는 반드시 json 형태로 대답해야되 내가 물어보면 그게 맞는 퀴즈를 내가 지정해준 형태로 json 만 리턴하면되며 언어는 한국어로 응답해줘"),
                Message(
                    role = "system", content = "[{\n" +
                            " 'studyName':string,\n" +
                            " 'question':string,\n" +
                            ...
                            "'choices':stirng[],\n" +
                            "'explanation':string\n" +
                            "}] 이런형태의 값으로 json 리스트 형태로 출력해줘 각 프로파티에 들어갈 값은 " +
                            "studyName=너가 생성한 퀴즈의 제목, question=문제내용," +
                            "answer=정답 객관식으므로 몇번인지 숫자만(1~5) ," +
                            "choices=객관식 문항 각 문항이 리스트에 들어간형태," +
                            "explanation=정답을 찾게 도움을 줄만한 힌트 단, 힌트에 정답이 될만한 문장이 포함되서는 안됨"
                ),
                Message(
                    role = "user",
    ...
    fun generateCodingTest(requestContentDto: RequestContentDto): GptCodingTestDto {
        val promptMessage = mutableListOf(
                Message(
                    role = "system",
                    content = "너는 반드시 json 형태로 대답해야되 내가 물어보면 그게 맞는 코딩테스트 문제를 내가 지정해준 형태로 json 만 리턴하면되"
                ),
                Message(
                    role = "system", content = "{\n" +
                            " 'title':string,\n" +
        ...
        if(requestContentDto.prompt!=null){
            promptMessage.add(
                Message(
                    role = "user",
                    content = "문제는 ${requestContentDto.prompt} 의 내용을 참고해서 작성해줘"
                )
            )
        }
        val request = GPTRequest(
    ...
    fun generateCodeReview(requestCodeReviewDto: RequestCodeReviewDto): GptCodeReviewDto {
        val request = GPTRequest(
            messages = listOf(
                Message(
                    role = "system",
                    content = "내가 코드를 보내주면 너는 해당코드의 개선점을 분석해서 코드 리뷰를 상세히 해줘"
                ),
                Message(
                    role = "system",
                    content = "{" +
                            ...
                            "title = 해당 코드 리뷰 제목" +
                            "description = 코디리뷰할 내용 내용이 길어질시 이스케이프를 통해 처리 3000자 이내 "
                ),
                Message(
                    role = "user",
